...
from dep_helper import get_word2id, get_vocab, get_label_list, get_pos2id, DummyLogger, is_local_main_process, is_main_process, get_gpu_usage
...
def train(args, acc: accelerate.Accelerator):
    ...
    broadcast_object_list(now_time)
    now_time = now_time[0]
    logger = logging.getLogger(f"{__name__}_seed_{args.seed}") if acc.is_local_main_process else DummyLogger()
    ...
    # optimizer = AdamW(dep_parser.parameters(), lr=args.learning_rate)
    ...
    num_of_no_improvement = torch.tensor(0, device=acc.local_process_index)
    ...
    dataloader = DataLoader(train_examples, batch_size=args.train_batch_size, num_workers=args.num_workers, collate_fn=lambda x: feature2input('cpu', convert_examples_to_features(x)))

    logger.info(f'process_idx: {acc.process_index}, first training example: {train_examples[0].text_a}')

    if acc.is_local_main_process:
        print(get_gpu_usage())
    sample_parameter = next(dep_parser.parameters())
    print(f'model parameter dtype: {sample_parameter.dtype}, device: {sample_parameter.device}')#, file=open('./out/param_type.txt', 'w'))

    dep_parser, dataloader, optimizer = acc.prepare(dep_parser, dataloader, optimizer)
    ...
    if args.do_train:
        ...
        for epoch in (trange(int(args.num_train_epochs), desc="Epoch") if acc.is_local_main_process else range(int(args.num_train_epochs))):
            # dep_parser.train()
            tr_loss = 0
            ...
            for step, batch in enumerate(pbar := (tqdm(dataloader) if acc.is_local_main_process else dataloader)):
                # dep_parser.train()
                ...
                # print(f'training batch size: {batch.input_ids.shape[0]}')
                with acc.accumulate(dep_parser):
                    input_ids, input_mask, l_mask, eval_mask, arcs, rels, word_ids, pos_ids, ngram_ids, ngram_positions, \
                    segment_ids, valid_ids = batch
                    if step == 0:
            ...
            if acc.is_local_main_process:
                # do evaluation
                dep_parser_to_eval = acc.unwrap_model(dep_parser)
                prediction = {}
                logger.info('\n')
                evaluator = Evaluator()
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    ...
                    input_ids, input_mask, l_mask, eval_mask, arcs, rels, word_ids, pos_ids, ngram_ids, ngram_positions, \
                    segment_ids, valid_ids = feature2input(acc.local_process_index, eval_features)
                ...
                if history['las'][-1] > best_dev_las:
                    ...
                    if args.model_name and acc.local_process_index == 0:
        ...
        if args.model_name is not None and acc.is_local_main_process:
...
def main(args):
    ...
    acc = accelerate.Accelerator(
        ...
        kwargs_handlers=[InitProcessGroupKwargs(timeout=datetime.timedelta(minutes=59))]
    )

    if args.do_train:
        return train(args, acc)
    elif args.do_test:
        test(args)
    else:
