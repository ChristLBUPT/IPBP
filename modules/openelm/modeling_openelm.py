...
class OpenELMMultiHeadCausalAttention(nn.Module):
    ...
    def forward(
        ...
        if not output_attentions:
            attn_weights = None
        return attn_output, attn_weights, past_key_value, queries if output_attention_queries else None
...
class OpenELMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, present_query = self.attn(
...
class OpenELMModel(OpenELMPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return tuple(
                v
                for v in [hidden_states, next_cache, all_hidden_states, all_self_attns, all_queries]
                if v is not None
            )
